...
public class ContentTipCommand implements Command {
	//페이지당 표시할 게시글의 수
		...
		public void execute(Model model, HttpSession httpSession, SqlSession sqlSession) {
			...
			viewDao.countingTipView(number);
			...
			String content = dto.gett_content().replaceAll("\n", "<br>");
			dto.sett_content(content);
			...
			model.addAttribute("comment_view", dao.commentSelect(t_num, offset, numOfTuplesPerPage));
